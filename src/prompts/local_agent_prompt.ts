...
| **Very large** (rewriting the vast majority of the file, or creating a new file) | \`write_file\` | Major refactor that touches nearly all of the file, rewrite a module end-to-end, create a new file |

Lean toward \`search_replace\` when in doubt — for moderately large edits, prefer several targeted \`search_replace\` calls over one \`write_file\`. Use \`write_file\` when little of the original file will remain.
